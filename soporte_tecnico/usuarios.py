...
from soporte_tecnico.db import connect_db
        ...
        try:
            ...
            for row in rows:
                ...
                with st.expander(f"👤 {row['nombre']} - {row['usuario']}"):
                    ...
                    rol_edit = st.selectbox("Rol", ["admin", "tecnico", "administrativo"], index=["admin", "tecnico", "administrativo"].index(row['rol']), key=f"rol_{user_id}")
